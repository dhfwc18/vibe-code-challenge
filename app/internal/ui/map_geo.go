...
func fillMapPolygon(screen *ebiten.Image, pts [][2]float32, col color.RGBA, ox, oy, sw, sh float32) {
	...
	p.Close()
	vs, is := p.AppendVerticesAndIndicesForFilling(nil, nil) //nolint:staticcheck // vector API migration deferred; current API works correctly
	r := float32(col.R) / 255
	g := float32(col.G) / 255
	b := float32(col.B) / 255
	for i := range vs {
...
func strokeMapPolygon(screen *ebiten.Image, pts [][2]float32, col color.RGBA, ox, oy, sw, sh float32) {
	...
	p.Close()
	vs, is := p.AppendVerticesAndIndicesForStroke(nil, nil, &vector.StrokeOptions{ //nolint:staticcheck // vector API migration deferred; current API works correctly
		Width:    1.5,
